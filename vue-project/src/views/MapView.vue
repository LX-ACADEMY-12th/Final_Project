...
const infoOverlay = ref(null); // ⬅ [유지] 호버 시 열릴 오버레이를 추적할 ref
...
const exhibitionMarkerImage = new window.kakao.maps.MarkerImage(
  '/museum.png', // ⬅ public 폴더의 '전시' 핀 이미지 경로
  imageSize,
  imageOption
);
...
const fieldTripMarkerImage = new window.kakao.maps.MarkerImage(
  '/experiment.png', // ⬅ public 폴더의 '답사' 핀 이미지 경로
  imageSize,
  imageOption
);
...
/* [삭제] 모든 오버레이 관련 스타일(.overlaybox, .custom-marker, 
  .current-location-dot, .customoverlay)을 
  <style scoped>에서 삭제합니다.
*/
</style>